...
func uploadAndTranscribe(ctx context.Context, filePath string, localEvents []string) error {
	...
	prompt := []genai.Part{
		...
		genai.Text("The audio of this meeting is in Brazilian Portuguese. Provide a complete transcription in Brazilian Portuguese separated by speaker (identifying them by their voices). Also create a structured executive summary and a clear list of action items (tasks) in Brazilian Portuguese." + eventsPrompt),
	}
